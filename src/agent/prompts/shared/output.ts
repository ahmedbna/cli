...
function finalizationPipeline(ctx: OutputContext): string {
  const steps: string[] = [];
  let i = 1;
  if (ctx.backend === 'convex') {
    steps.push(`      ${i++}. \`npx convex dev --once\`              — initialize Convex project`);
  } else if (ctx.backend === 'supabase') {
    steps.push(`      ${i++}. \`supabase db push\`                    — apply migrations`);
  }
  steps.push(`      ${i++}. \`tsc --noEmit\` + autofix              — full TypeScript check`);
  steps.push(`      ${i++}. \`git init && git add . && git commit\` — snapshot the project`);
  if (ctx.backend === 'convex') {
    steps.push(`      ${i++}. \`npx @convex-dev/auth\` + env vars     — configure auth`);
  }
  if (ctx.frontend === 'expo') {
    steps.push(`      ${i++}. \`npx expo run:ios\` / \`run:android\`    — launch in simulator`);
  }
  return steps.join('\n');
}

function deferredCommands(ctx: OutputContext): string {
  const lines: string[] = [];
  if (ctx.frontend === 'expo') {
    lines.push('- DO NOT run `npx create-expo-app` or scaffolding — template is pre-copied');
  }
  lines.push("- DO NOT run `npm install` — it's already running in the background");
  if (ctx.backend === 'convex') {
    lines.push('- DO NOT run `npx convex dev` / `npx convex deploy` — deferred to finalization');
    lines.push('- DO NOT run `npx @convex-dev/auth` — deferred to finalization');
  }
  lines.push('- DO NOT run `git init` / `git add` / `git commit` — deferred to finalization');
  lines.push('- DO NOT run `tsc` / `npx tsc` — deferred to finalization');
  if (ctx.frontend === 'expo') {
    lines.push('- DO NOT run `npx expo run:ios` / `run:android` — deferred to finalization');
    lines.push('- ONLY use `runCommand` for `npx expo install <pkg>` when adding packages not in the template');
  }
  return lines.join('\n');
}

function devBuildAwareness(ctx: OutputContext): string {
  if (ctx.frontend !== 'expo') return '';
  return `
  ## Dev Build Awareness
...
function planningOrder(ctx: OutputContext): string {
  const lines: string[] = [
  ...
  lines.push(
    ctx.backend === 'convex'
      ...
      : ctx.backend === 'supabase'
        ? '  2. **Lookup docs** — use `lookupDocs` for advanced Supabase or Expo features'
        : '  2. **Lookup docs** — use `lookupDocs` for advanced features',
  );
  lines.push('  3. **Theme** — write `theme/colors.ts` with a unique palette + RADIUS/SPACING tokens');
  lines.push('  4. **UI components** — create/update reusable components in `components/ui/`');
  if (ctx.backend === 'convex') {
    lines.push('  5. **Schema** — design the Convex data model (keep ...authTables and users table)');
    lines.push('  6. **Functions** — write queries and mutations');
  } else if (ctx.backend === 'supabase') {
    lines.push('  5. **Schema** — design the Supabase data model (migrations + RLS policies)');
    lines.push('  6. **Functions** — write queries and RPCs/edge functions');
  } else {
    lines.push('  5. **Data model** — design local data model');
    lines.push('  6. **Data access** — wire the data layer');
  }
  lines.push('  7. **Screens** — build screens using the UI components');
  if (ctx.frontend === 'expo') {
    lines.push('  8. **Packages** — only run `npx expo install <pkg>` for NEW native packages, near the end');
  }
  lines.push('  9. **ARCHITECTURE.md** — ALWAYS write this as the FINAL step');
  return lines.join('\n');
}

function lockedFilesSection(ctx: OutputContext): string {
  const lines: string[] = [];
  if (ctx.frontend === 'expo') {
    lines.push('  - `components/auth/authentication.tsx` — only style/theme colors');
    lines.push('  - `components/auth/singout.tsx` — only style/theme colors');
  }
  if (ctx.backend === 'convex') {
    lines.push('  - `convex/auth.config.ts` — never modify');
    lines.push('  - `convex/auth.ts` — never modify (except style of loggedInUser if needed)');
  }
...
export function outputInstructions(ctx: OutputContext) {
  return stripIndents`
<output_instructions>
  ## Communication
...
`
    : ''
}  - **Everything else is deferred.** Do NOT run ${ctx.backend === 'convex' ? '`npx convex dev`, `npx convex deploy`,\n    `npx @convex-dev/auth`, ' : ''}\`git init\`, \`tsc\`, or ${ctx.frontend === 'expo' ? '`npx expo run:*`' : 'other long-running commands'}. After you
    finish, the CLI automatically runs these in this exact order:
${finalizationPipeline(ctx)}
...
${ctx.frontend === 'expo' ? '  2. Any `npx expo install` calls come near the end\n' : ''}  3. ARCHITECTURE.md is the very last file you write
...
${deferredCommands(ctx)
  .split('\n')
  .map((l) => '  ' + l)
  .join('\n')}
${devBuildAwareness(ctx)}
