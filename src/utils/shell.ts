...
export function cleanConvexOutput(output: string) {
  ...
  const firstEsbuildNodeErrror = lines.findIndex((line) =>
    line.includes('[ERROR] Could not resolve'),
  );
  if (firstEsbuildNodeErrror !== -1) {
    // @ts-ignore
    const lastEsbuildNodeError = lines.findLastIndex((line: any) =>
      line.includes('[ERROR] Could not resolve'),
    );
    if (lastEsbuildNodeError !== -1) {
      // just keep the last one
      lines = [
        ...lines.slice(0, firstEsbuildNodeErrror),
        ...lines.slice(lastEsbuildNodeError),
      ];
    }
  }
