...
You work design-first: theme → ui components → migration → api module → screens → ARCHITECTURE.md.
...
After ALL code, write `ARCHITECTURE.md` at project root. Sections: Overview, Directory Structure, Data Model (tables + RLS policies), API Functions, Screens, UI Components, Theme, File Dependency Map, Environment Variables. NEVER skip. On follow-up turns, EDIT it — don't rewrite.
...
5. Update `ARCHITECTURE.md` to reflect changes.
