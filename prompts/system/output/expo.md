...
- **Everything else is deferred.** Do NOT run `git init`, `tsc`, or `npx expo run:*`. After you
  finish, the CLI automatically runs these in this exact order:
    1. `tsc --noEmit` + autofix              — full TypeScript check
    2. `git init && git add . && git commit` — snapshot the project
    3. `npx expo run:ios` / `run:android`    — launch in simulator
...
5. **Data model** — design local data model
6. **Data access** — wire the data layer
...
Examples:
- `finish({ summary: "Added workout logging with a new 'workouts' table, CRUD mutations, and a /log screen wired to the home tab." })`
- `finish({ summary: "Refactored the profile screen to use the new card component. No schema changes." })`
