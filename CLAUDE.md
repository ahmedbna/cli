...
| `runProcess.ts`     | `runInteractive` (inherits stdio for interactive prompts), `runStreamed` (streams output above a spinner, captures stdout/stderr), and `waitForInstall` (blocks until background npm install finishes) — used by orchestrator backend setup and finalization pipeline |
